...
from Model import RNN_Model, Transformer_Model_5, Transformer_Model_2, Transformer_Model_3, TransGRU_1
#这边的模型当中采用RNN_model,Transformer模型
import random
import pickle
...
class Amino(Dataset):
    ...
    def __init__(self, datas):
        ...
        self.datas = datas
        # 创建序列上核苷酸和数字对应的字典
        self.d_dict = {'A': 0, 'C': 1, 'D': 2, 'E': 3, 'F': 4, 'G': 5, 'H': 6, 'I': 7, 'K': 8, 'L': 9,
...
def get_data(filename, train_rate_used=0, val_rate_used=0, batch=0, datatype='N'):
    '''
    Q：为什么要对数据进行这样的分层操作，整理为什么要选择 N和 A
    :param filename: 文件路径
...
def train_and_validate_model(model, train_dataloader, val_dataloader, criterion, optimizer, device, epochs,
    ...
    scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=6, threshold=1e-3,
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, data in enumerate(train_dataloader):
            ...
            train_loss_kl.append(kl_div.item())
        ...
        # 计算每个 epoch 中的平均损失
        avg_train_loss_kl = torch.mean(torch.tensor(train_loss_kl))
        train_kl_res.append(avg_train_loss_kl.item())  # 存储每个epoch的分布误差
        ...
        with torch.no_grad():
            ...
            avg_val_kl = torch.mean(torch.tensor(kl_div))
            val_kl_res.append(avg_val_kl.item())
        ...
        print(f'Epoch {epoch + 1}, '
              ...
              f'Train-KL:{avg_train_loss_kl:.5f},'
              f'Train RMSE: {avg_train_loss2_loss:.3f}, '
              f'Validation SmoothL1: {avg_val_loss:.5f}, '
              f'Validation-KL:{avg_val_kl:.5f},'
              f'Val RMSE: {avg_val_loss2:.3f}'
              )

        # 返回 loss 列表
    return train_loss_kl, train_loss_res, train_loss2_res, val_loss_kl, val_loss_res, val_loss2_res, save_model_as_filename
...
def test_model(model_path, model, test_dataloader, criterion, device):
    ...
    kl_div_weight = 0.8  # 权重可以根据需求进行调整
    Rmse_weight = 1.5
    ...
    print(f'Test-KL: {np.mean(test_kl):.5f},'
        f'Test SmoothL1: {np.mean(test_loss):.3f}, '
          f'Test RMSE: {np.mean(test_rmse):.3f}'
          )

    return np.mean(kl_div), np.mean(test_loss), np.mean(test_rmse)


def save_data_to_pickle(save_path, train_loss_res, val_loss_res, train_loss2_res, val_loss2_res, test_loss, test_rmse):
    # 数据存储
    data = {
        "train_kl_res": train_kl_res,
        "val_kl_res": val_kl_res,
...
if __name__ == '__main__':
    ...
    # 1 读取配置文件 trans
    config = json.load(open('Data/Params/model_tsf_3.json', 'r', encoding='UTF-8'))  # 打开数据

    # 选择模型
    selected_model = "model-T5-K5"
    ...
    model = Transformer_Model_5(vocab_size=vs, embed_dim=ed, hidden_size=hs, num_layers_1=nl1, num_layers_2=nl2,
                                dropout=dp).to(device)  # 0.96--0.91
    # model = TransGRU_1(vocab_size=vs, embed_dim=ed, hidden_size=hs, num_layers_1=nl1, num_layers_2=nl2,
    ...
    # 在随机生成的数据上进行预测
    # 导入数据
    # get_pre_data = json.load(open('PreData&Res/Data/100wData-truth.json', 'r'))
    # pre_dataset = Amino(get_pre_data)
    # pre_dataloader = DataLoader(pre_dataset, shuffle=False, batch_size=256)  # 每个 epoch 再打乱
    #
    # # 导入训练好的预测模型，并通过预测模型对随机生成的蛋白质序列进行预测，这里的model文件并不是按照真实数据修改完成的
    ...
    # with open('Data/Result/TransGru/2GRU+Trans/A/5-fold/test/100w-result-truth.json', 'w') as f:
    #     json.dump(get_pre_data, f)

    # ———————————————————————————————————————————————————测试数据预测—————————————————————————————————————————————————————
    # 在测试集上进行预测
    # 导入模型
    # ckpt = torch.load("Data/Result/Transformer3/A/model.pt", map_location=device)  # 添加map_location参数
    #
    # model.load_state_dict(ckpt)
    # model.eval()
    #
