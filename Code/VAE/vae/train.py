...
def train(
    ...
    patience=10,  #10 Number of epochs to continue training for after convergence
    min_epochs=10,#10
    max_epochs=30, #30
    ...
    batch_size = 64 #batches_list[0][0].shape[0]
    ...
    preds_predata = predict(model, predata_df,Y=None,AA=None)
    ...
    return model, preds_df, model_outdir,preds_predata
...
def train_new(
    ...
    train_batches,
    val_batches,
    array,  # col： AA_sequence or seq
    outpath,  # the model sava path
    train_df,
    optimizer=None,
    model_outdir=None,  # Directory for saving model and logs
    loss_weights=[1.0, 0.5, 0.1],
    ...
    # also sort of nicer not to have to re-input parameters repeatedly...
    batches_list = list(train_batches.as_numpy_iterator())
    batch_size = 16 #batches_list[0][0].shape[0]
    last_batch_size = batches_list[-1][0].shape[0]
    num_batches = len(batches_list)
    num_training_samples = (num_batches - 1) * batch_size + last_batch_size
...
def pretrain(
    ...
    AA = preddata_df["AA_sequence"].values
    X = preddata_df[[col for col in  preddata_df.columns if re.match(r"x\d+", col)]].values
    Y = preddata_df['nor_package'].values


    model_load_path = model_dir
    loaded_model = tf.saved_model.load(model_load_path)
    preds_df = 0
