...
def cnn_aa_model(L1=140, L2=20):
    """
    For AAV data,  aa and nor_pakage, 
    Here, parent_model is set as "CNN" model for learning the feature of data 
    """
    learning_rate = 0.0005
    model = Sequential()
    model.add(Conv2D(filters=L1, kernel_size=(3, 3), activation='relu', input_shape=(7, 20, 1)))  # Assuming input shape is (7, 20, 1)
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Flatten())
    model.add(Dense(units=32, activation='relu'))  # Adjust units based on your task
...
def cnn_seq_model(L1=420, L2=60):
    """
    For AAV data,  aa and nor_pakage, 
    Here, parent_model is set as "CNN" model for learning the feature of data
    """
    learning_rate = 0.0005
    model = Sequential()
    model.add(Conv2D(filters=L1, kernel_size=(3, 3), activation='relu', input_shape=(21, 20, 1)))  # Assuming input shape is (7, 20, 1)
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Flatten())
    model.add(Dense(units=32, activation='relu'))  # Adjust units based on your task
    model.add(Dense(units=1))  # Output layer
